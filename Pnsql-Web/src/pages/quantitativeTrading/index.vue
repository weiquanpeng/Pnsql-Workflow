    ...
    <div class="space-container">
      <div class="card-layout">
        ...
        <div class="additional-options">
          <t-date-picker v-model="selectedDate" placeholder="选择日期" @change="handleDateChange" style="margin-right: 10px" />
...
import { getDragon_queryDate, getFollowStock, getStockF0, uptFollowStatus, getAnnualMovingAverage, getSixtyMovingAverage } from '@/api/services/trading';

import TradingDiglog from './TradingDiglog.vue';

// 定义格式化日期的函数
const cacheSize = 12; // 每次缓存 10 支股票
...
const loadStockData = async (stockTicker: string, updateChartFlag = true) => {
  if (selectedStock.value !== stockTicker) return;
    ...
    try {
        ...
        if (response?.data?.data) {
            const newData = response.data.data.map((item) => ({
                ...
                f4: item.f4,
                f5: item.f5,
                f12: item.f12 === 0 ? null : item.f12,
                f13: item.f13 === 0 ? null : item.f13,
                f14: item.f14 === 0 ? null : item.f14,
                f15: item.f15 === 0 ? null : item.f15,
                f16: item.f16 === 0 ? null : item.f16,
                f17: item.f17 === 0 ? null : item.f17,
                f18: item.f18 === 0 ? null : item.f18,
            }));
