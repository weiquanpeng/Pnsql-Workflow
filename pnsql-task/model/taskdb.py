...
class TaskDB(BaseDb):
    ...
    def get_subtask_by_id(self, id):
        sql = "select * from subtask_config where id=%s"
    ...
    def get_approve_subtasks(self):
        sql = (
            ...
            "    SELECT s.id, s.create_time, s.update_time, s.title, s.type, s.owner, s.approver, "
            "           s.task_id, s.status, s.paras, s.task_describe, "
            ...
            "      AND s.status!= 'done'"
            ") "
            "SELECT st.id, st.create_time, st.update_time, st.title, st.type, st.owner, st.approver, "
            "       st.task_id, st.status, st.paras, st.task_describe "
    ...
    def add_subtask(self, subtasks):
        ...
        INSERT INTO subtask_config (
            task_id, title, owner, approver, status, paras, task_describe, type
        ) VALUES (
            %s, %s, %s, %s, %s, %s, %s, %s
        )
        """
        try:
            for subtask in subtasks:
                ...
                parameter_json = json.dumps(subtask['paras'], ensure_ascii=False)
                # 构造插入数据的元组
                data = (
                    subtask['taskid'],
                    subtask['title'],
